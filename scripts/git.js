	...
	.then(function (data) {
		...
		for (let i of data) {
			if (i.type == 'PushEvent') {
				...
				let combine_info = `
				<table style="width: 100%; height: 15%; overflow: hidden; background-color: rgba(255, 255, 255, 0.8); border-radius: 5px; box-shadow: 2px 2px 2px 2px rgba(0, 0, 0, 0.5);">
					<tbody>
						<tr>
							<td style="text-align: center; background-image: url('${target_name}'); background-position: center; background-size: cover; background-repeat: no-repeat">
								<table style="width: 100%; height: 100%;">
									<tbody>
										<tr>
											<td style="width: 50%; height: 100%; text-align: left; padding-left: 5%;">
												<a href="${author.link}" target="_blank" class="pulse">${author.name}</a><br />
												${title.name}<br />
												${time}
											</td>
		...
		<table style="width: 90%; height: 90%; overflow: hidden;">
			<tbody>
				<tr>
					<td style="width: 42%; height: 100%; overflow: hidden;">${array.slice(0,5).join(' ')}</td>
					<td style="width: 6%;"></td>
					<td style="width: 42%; height: 100%; overflow: hidden;">${array2.slice(0,5).join(' ')}</td>
				</tr>
			</tbody>
		</table>
			`;
